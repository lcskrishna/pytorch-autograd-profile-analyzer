...
nvprof_path="/data/work/megatron-bringup/profile/Megatron/timeline_bs2_768.nvprof"
...
start_rows=conn.execute("SELECT * FROM CUPTI_ACTIVITY_KIND_MARKER WHERE name != 0").fetchall()
end_rows=conn.execute("SELECT * FROM CUPTI_ACTIVITY_KIND_MARKER WHERE name = 0").fetchall()
...
for r in end_rows:
    end_dict[r["id"]]=r
print(len(start_dict),len(end_dict))

for row_id,row in start_dict.items():
#     eprintRow(row)
    
    # TODO: SO INEFFICIENT
    if row_id in end_dict:
        end_row=end_dict[row_id]
    else:
        end_row=None
...
kernel_timeline={"traceEvents":traceEvents}
json.dump(kernel_timeline, open("timeline_dump.json","w"))
